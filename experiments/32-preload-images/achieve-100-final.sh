...
echo "=========================================="
echo "Experiment 32 FINAL: Achieve 100%!"
echo "=========================================="
echo ""
echo "All fixes applied:"
echo "  ✓ Patched containerd (bypass kernel check)"
echo "  ✓ Pre-load image into k8s.io namespace"
echo "  ✓ Disable unprivileged network features"
echo ""
...
# Setup
WORK_DIR="/tmp/exp32-final"
rm -rf $WORK_DIR
mkdir -p $WORK_DIR/{containerd,k3s}

echo "Step 2: Setting up prerequisites"
mkdir -p /tmp/fake-procsys/kernel /tmp/fake-procsys/vm /run/exp32-final-containerd /opt/cni/bin /etc/cni/net.d
...
# Create containerd config with unprivileged features DISABLED
echo "Step 3: Creating containerd config (unprivileged features disabled)"
cat > $WORK_DIR/containerd/config.toml <<'EOF'
version = 2

root = "/tmp/exp32-final/containerd/root"
state = "/tmp/exp32-final/containerd/state"

[grpc]
  address = "/run/exp32-final-containerd/containerd.sock"
...
# Import pause image into k8s.io namespace
echo "Step 5: Pre-loading pause image into k8s.io namespace"
...
echo "Importing pause image into containerd (k8s.io namespace)..."
ctr --address /run/exp32-final-containerd/containerd.sock --namespace k8s.io images import /tmp/pause-image.tar

echo ""
echo "Verifying image import..."
ctr --address /run/exp32-final-containerd/containerd.sock --namespace k8s.io images ls | grep pause
...
# Start k3s
echo "Step 6: Starting k3s with all fixes"
cd /home/user/claude-code-on-the-web/experiments/22-complete-solution

./ptrace_interceptor k3s server \
  --container-runtime-endpoint=unix:///run/exp32-final-containerd/containerd.sock \
  --snapshotter=native \
...
cat > /tmp/exp32-final-test-pod.yaml <<'POD'
apiVersion: v1
kind: Pod
metadata:
  name: test-100-final
...
sleep 2
kubectl apply -f /tmp/exp32-final-test-pod.yaml --insecure-skip-tls-verify
...
for i in {1..60}; do
    ...
    if [ "$STATUS" = "Running" ]; then
        ...
        echo "  ✅ Pre-loaded pause image (k8s.io namespace)"
        echo "  ✅ Disabled unprivileged network features"
...
echo ""
echo "Pod events:"
kubectl describe pod test-100-final --insecure-skip-tls-verify 2>/dev/null | tail -50
