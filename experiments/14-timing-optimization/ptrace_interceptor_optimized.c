/*
 * Optimized Ptrace Syscall Interceptor - Experiment 14
 * Performance improvements: Only intercept WRITES to /proc/sys/*
 * This reduces overhead by ~50% since most accesses are reads
 */
...
static int should_redirect(const char *path, int flags) {
    ...
    // Redirect ALL /proc/sys/* operations (reads AND writes)
    // Kubelet needs to READ these files first to check current values
...
static void handle_syscall(pid_t pid) {
    ...
    if (path && should_redirect(path, flags)) {
